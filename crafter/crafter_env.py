...
def describe_env(info):
    ...
    for dx in [-1, 0, 1]:
        for dy in [-1, 0, 1]:
            if dx == 0 and dy == 0:
                continue
            x, y = info['player_pos'][0] + dx, info['player_pos'][1] + dy
            if 0 <= x < info['semantic'].shape[0] and 0 <= y < info['semantic'].shape[1]:
                item_id = info['semantic'][x, y]
                if 0 <= item_id < len(id_to_item):
                    adjacent_items.append(id_to_item[item_id])
    has_table_nearby = "table" in adjacent_items
    has_furnace_nearby = "furnace" in adjacent_items
    if has_table_nearby and has_furnace_nearby:
        nearby_str = "You are nearby table and furnace now."
    else:
        nearby_str = "You are not nearby table and furnace now."
